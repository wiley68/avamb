      ...
      <div class="w-full flex items-center justify-between">
        <div class="">
          <button
            ...
            @click.stop="store.methods.createAvansova(store.state.current_oferti)"
          >
            <svg
        ...
        <div class="font-medium">Авансови фактури</div>
      </div>
    </div>
  </div>
  <div class="px-5 py-4">
    <ul class="mb-6">
      <li v-for="faktura in getAvansovi()" :key="faktura.id" class="-mx-2 mb-1">
        <button
          class="flex flex-col w-full p-1 text-left bg-gray-100 rounded border border-gray-200"
          @click.stop="editAvansova(faktura.idnomber)"
        >
  ...
  setup() {
    ...
    const getAvansovi = () => {
      if (store.state.current_oferti !== 0) {
        return store.state.avansovi.filter(
          (element) => element.offer_idnomber == store.state.current_oferti
        );
      } else {
        return store.state.avansovi;
      }
    };

    const editAvansova = (avansova_idnomber) => {
      store.state.current_avansovi = avansova_idnomber;
      setTimeout(() => {
