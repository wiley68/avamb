<template>
  <div
    ...
    :class="store.state.modul1SidebarOpen ? 'translate-x-1/3' : 'translate-x-0'"
    v-if="modul1"
  >
      ...
      >
        <div class="flex">
          <button
            ...
            :aria-expanded="store.state.modul1SidebarOpen"
          >
          ...
          <button
            ...
            @click.stop="updateModul1(modul1.id)"
          >
            <svg
          ...
          <ModalBlank
            id="success-modal"
            :modalOpen="store.state.successUpdateModul1"
            @close-modal="store.methods.changeSuccessUpdateModul1(false)"
          >
              ...
              <div>
                ...
                <div class="flex flex-wrap justify-end space-x-2">
                  <button
                    ...
                    @click.stop="store.methods.changeSuccessUpdateModul1(false)"
                  >
                    Затвори
                  </button>
                </div>
          ...
          <ModalBlank
            id="danger-modal"
            :modalOpen="store.state.deleteModul1Modal"
            @close-modal="store.methods.changeDeleteModul1Modal(false)"
          >
            <div class="p-5 flex space-x-4">
              <div
              ...
              <div>
                ...
                <div class="flex flex-wrap justify-end space-x-2">
                  <button
                    class="btn-sm border-gray-200 hover:border-gray-300 text-gray-600"
                    @click.stop="store.methods.changeDeleteModul1Modal(false)"
                  >
                    Откажи
                  </button>
                  <button
                    class="btn-sm bg-red-500 hover:bg-red-600 text-white"
                    @click.stop="deleteModul1()"
                  >
                    Изтрий
                  </button>
                </div>
...
export default {
  ...
  setup() {
    ...
    const updateModul1 = (modul1_id) => {
      store.methods.saveModul1(modul1_id)
    }

    const deleteModul1Check = () => {
      store.methods.changeDeleteModul1Modal(true)
    }

    const deleteModul1 = () => {
      const deleted_id = modul1.value.id
      store.methods.deleteModul1(deleted_id)
    }
