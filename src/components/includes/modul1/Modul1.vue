...
export default {
  ...
  setup() {
    ...
    watch(offer_id, async (newOfferId, oldClient) => {
      modul1.value.offer_id = newOfferId
      ...
      if (offer_selected) {
        ...
        if (client_selected) {
          modul1.value.client_id = client_selected.id
          modul1.value.client_name = clentName(
    ...
    const changeClient = () => {
      store.methods.getObekti(modul1.value.client_id)
    }

    watch(client, async (newClient, oldClient) => {
      modul1.value.client_id = newClient.id
      modul1.value.client_name = clentName(
