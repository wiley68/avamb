<template>
  <div class="flex flex-col py-3 w-full">
    ...
    <div class="flex flex-col mb-1">
      ...
      <select
        class="w-full text-sm border rounded-sm border-gray-100 p-1 bg-blue-300"
        v-model="modul2.statusi_id"
      >
        <option
          v-for="item in modul2.select_statusi"
          :key="item.id"
          :value="item.id"
        >
          {{ item.name }}
    ...
    <div class="flex flex-col mb-1">
      <textarea
        rows="8"
        class="w-full text-sm border rounded-sm border-gray-100 p-1"
        v-model="modul2.text2"
      ></textarea>
    </div>
    <div class="flex flex-col mb-1">
      <span class="p-1 text-sm">{{ modul2.modul2_info1 }}</span>
      <input
        type="text"
        class="w-full text-sm border rounded-sm border-gray-100 p-1"
        v-model="modul2.info1"
      />
    </div>
    <div class="flex flex-col mb-1">
      <span class="p-1 text-sm">{{ modul2.modul2_info2 }}</span>
      <input
        type="text"
        class="w-full text-sm border rounded-sm border-gray-100 p-1"
        v-model="modul2.info2"
      />
    </div>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    const offer_id = ref(modul2.value.offer_id)
    const shablon1 = ref('')
    const shablon2 = ref('')
    ...
    watch(offer_id, async (newOfferId, oldClient) => {
      modul2.value.offer_id = newOfferId
      ...
      if (offer_selected) {
        ...
        if (client_selected) {
          modul2.value.client_id = client_selected.id
          modul2.value.client_name = clentName(
    ...
    const changeText1 = () => {
      modul2.value.text1 = shablon1.value
    }

    const changeText2 = () => {
      modul2.value.text2 = shablon2.value
    }
    ...
    const changeClient = () => {
      store.methods.getObekti(modul2.value.client_id)
    }

    watch(client, async (newClient, oldClient) => {
      modul2.value.client_id = newClient.id
      modul2.value.client_name = clentName(
