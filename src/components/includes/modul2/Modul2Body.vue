<template>
  <div
    ...
    :class="store.state.modul2SidebarOpen ? 'translate-x-1/3' : 'translate-x-0'"
    v-if="modul2"
  >
      ...
      >
        <div class="flex w-full items-center justify-between">
          <button
            ...
            @click.stop="store.methods.toggleModul2Sidebar()"
            aria-controls="inbox-sidebar"
            :aria-expanded="store.state.modul2SidebarOpen"
          >
          ...
          <button
            ...
            @click.stop="updateModul2(modul2.id)"
          >
            <svg
          ...
          <ModalBlank
            id="success-modal"
            :modalOpen="store.state.successUpdateModul2"
            @close-modal="store.methods.changeSuccessUpdateModul2(false)"
          >
              ...
              <div>
                ...
                <div class="flex flex-wrap justify-end space-x-2">
                  <button
                    ...
                    @click.stop="store.methods.changeSuccessUpdateModul2(false)"
                  >
                    Затвори
                  </button>
                </div>
          ...
          <ModalBlank
            id="danger-modal"
            :modalOpen="store.state.deleteModul2Modal"
            @close-modal="store.methods.changeDeleteModul2Modal(false)"
          >
            <div class="p-5 flex space-x-4">
              <div
              ...
              <div>
                ...
                <div class="flex flex-wrap justify-end space-x-2">
                  <button
                    class="btn-sm border-gray-200 hover:border-gray-300 text-gray-600"
                    @click.stop="store.methods.changeDeleteModul2Modal(false)"
                  >
                    Откажи
                  </button>
                  <button
                    class="btn-sm bg-red-500 hover:bg-red-600 text-white"
                    @click.stop="deleteModul2()"
                  >
                    Изтрий
                  </button>
                </div>
...
export default {
  ...
  components: { ModalBlank, Modul2 },

  setup() {
    const store = inject('store')

    const modul2 = computed(() => {
      return store.state.moduli2_temp.find(
        (element) => element.id == store.state.current_modul2_id
      )
    })

    const updateModul2 = (modul2_id) => {
      store.methods.saveModul2(modul2_id)
    }

    const deleteModul2Check = () => {
      store.methods.changeDeleteModul2Modal(true)
    }

    const deleteModul2 = () => {
      const deleted_id = modul2.value.id
      store.methods.deleteModul2(deleted_id)
    }
