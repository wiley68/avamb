<template>
  <div
    ...
    :class="store.state.modul3SidebarOpen ? 'translate-x-1/3' : 'translate-x-0'"
    v-if="modul3"
  >
      ...
      >
        <div class="flex w-full items-center justify-between">
          <button
            ...
            @click.stop="store.methods.toggleModul3Sidebar()"
            aria-controls="inbox-sidebar"
            :aria-expanded="store.state.modul3SidebarOpen"
          >
          ...
          <button
            ...
            @click.stop="updateModul3(modul3.id)"
          >
            <svg
          ...
          <ModalBlank
            id="success-modal"
            :modalOpen="store.state.successUpdateModul3"
            @close-modal="store.methods.changeSuccessUpdateModul3(false)"
          >
              ...
              <div>
                ...
                <div class="flex flex-wrap justify-end space-x-2">
                  <button
                    ...
                    @click.stop="store.methods.changeSuccessUpdateModul3(false)"
                  >
                    Затвори
                  </button>
                </div>
          ...
          <ModalBlank
            id="danger-modal"
            :modalOpen="store.state.deleteModul3Modal"
            @close-modal="store.methods.changeDeleteModul3Modal(false)"
          >
            <div class="p-5 flex space-x-4">
              <div
              ...
              <div>
                ...
                <div class="text-sm mb-10">
                  <div class="space-y-2">
                    <p>Желаете ли да изтриете документа?</p>
                  </div>
                </div>
                <div class="flex flex-wrap justify-end space-x-2">
                  <button
                    class="btn-sm border-gray-200 hover:border-gray-300 text-gray-600"
                    @click.stop="store.methods.changeDeleteModul3Modal(false)"
                  >
                    Откажи
                  </button>
                  <button
                    class="btn-sm bg-red-500 hover:bg-red-600 text-white"
                    @click.stop="deleteModul3()"
                  >
                    Изтрий
                  </button>
                </div>
...
export default {
  ...
  components: { ModalBlank, Modul3 },

  setup() {
    const store = inject('store')

    const modul3 = computed(() => {
      return store.state.moduli3_temp.find(
        (element) => element.id == store.state.current_modul3_id
      )
    })

    const updateModul3 = (modul3_id) => {
      store.methods.saveModul3(modul3_id)
    }

    const deleteModul3Check = () => {
      store.methods.changeDeleteModul3Modal(true)
    }

    const deleteModul3 = () => {
      const deleted_id = modul3.value.id
      store.methods.deleteModul3(deleted_id)
    }
