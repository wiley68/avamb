<template>
  <div
    ...
    :class="store.state.modul4SidebarOpen ? 'translate-x-1/3' : 'translate-x-0'"
    v-if="modul4"
  >
      ...
      >
        <div class="flex w-full items-center justify-between">
          <button
            ...
            @click.stop="store.methods.toggleModul4Sidebar()"
            aria-controls="inbox-sidebar"
            :aria-expanded="store.state.modul4SidebarOpen"
          >
          ...
          <button
            ...
            @click.stop="updateModul4(modul4.id)"
          >
            <svg
          ...
          <ModalBlank
            id="success-modal"
            :modalOpen="store.state.successUpdateModul4"
            @close-modal="store.methods.changeSuccessUpdateModul4(false)"
          >
              ...
              <div>
                ...
                <div class="flex flex-wrap justify-end space-x-2">
                  <button
                    ...
                    @click.stop="store.methods.changeSuccessUpdateModul4(false)"
                  >
                    Затвори
                  </button>
                </div>
          ...
          <ModalBlank
            id="danger-modal"
            :modalOpen="store.state.deleteModul4Modal"
            @close-modal="store.methods.changeDeleteModul4Modal(false)"
          >
            <div class="p-5 flex space-x-4">
              <div
              ...
              <div>
                ...
                <div class="flex flex-wrap justify-end space-x-2">
                  <button
                    class="btn-sm border-gray-200 hover:border-gray-300 text-gray-600"
                    @click.stop="store.methods.changeDeleteModul4Modal(false)"
                  >
                    Откажи
                  </button>
                  <button
                    class="btn-sm bg-red-500 hover:bg-red-600 text-white"
                    @click.stop="deleteModul4()"
                  >
                    Изтрий
                  </button>
                </div>
...
export default {
  ...
  components: { ModalBlank, Modul4 },

  setup() {
    const store = inject('store')

    const modul4 = computed(() => {
      return store.state.moduli4_temp.find(
        (element) => element.id == store.state.current_modul4_id
      )
    })

    const updateModul4 = (modul4_id) => {
      store.methods.saveModul4(modul4_id)
    }

    const deleteModul4Check = () => {
      store.methods.changeDeleteModul4Modal(true)
    }

    const deleteModul4 = () => {
      const deleted_id = modul4.value.id
      store.methods.deleteModul4(deleted_id)
    }
