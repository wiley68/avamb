  ...
  <div>
    ...
    <div class="px-5 py-4">
      <ul class="mb-6">
        ...
        <li class="-mx-2 mb-1">
          <button
            @click.stop="store.methods.changePage('Proformi')"
            class="w-full p-2 rounded-lg border-2 border-orange-600"
          >
            <div class="flex items-center">
              <div
        ...
        <li class="-mx-2 mb-1">
          <button
            @click.stop="store.methods.changePage('Avansovi')"
            class="w-full p-2 rounded-lg border-2 border-orange-600"
          >
            <div class="flex items-center">
              <div
              ...
              <div class="ml-3">
                <span class="text-lg font-medium text-gray-900">Авансови фактури</span>
              </div>
            </div>
          </button>
        </li>
        <li class="-mx-2 mb-1">
          ...
          >
            <div class="flex items-center">
              ...
              <div class="ml-3">
                <span class="text-lg font-medium text-gray-900">Фактури (продажби)</span>
              </div>
            </div>
          </button>
        </li>
...
export default {
  ...
  setup() {
    ...
    const fakturiNomber = () => {
      const fakturi_nomber = store.state.fakturi.filter(
        (element) => element.offer_idnomber == store.state.current_oferti
      );
      return fakturi_nomber ? fakturi_nomber.length : 0;
    };
    ...
    const avansoviNomber = () => {
      const avansovi_nomber = store.state.avansovi.filter(
        (element) => element.offer_idnomber == store.state.current_oferti
      );
      return avansovi_nomber ? avansovi_nomber.length : 0;
    };

    const createFaktura = () => {
      const offer = store.state.alloffers.find(
        (element) => element.idnomber == store.state.current_oferti
      );
      store.methods.createFaktura(offer);
    };
