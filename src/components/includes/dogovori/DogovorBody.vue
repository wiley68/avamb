  ...
  <div class="flex relative h-14 bg-gray-50 border-b border-gray-200 mb-1">
    <div class="flex justify-between items-center w-full mr-4">
      <button class="md:hidden absolute top-4 left-4 sm:left-6 text-white opacity-80 hover:opacity-100"
        @click.stop="store.methods.toggleDogovorSidebar()" aria-controls="about-sidebar"
        :aria-expanded="store.state.dogovorSidebarOpen">
        <span class="sr-only">Договор</span>
        <svg class="w-6 h-6 fill-current" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <path class="text-gray-500" d="M10.7 18.7l1.4-1.4L7.8 13H20v-2H7.8l4.3-4.3-1.4-1.4L4 12z" />
        </svg>
      </button>
      ...
      <div class="flex items-center">
        <button
          ...
          aria-controls="success-modal" @click.stop="updateDogovor()">
  ...
  <div v-if="store.state.dogovorEtap == 1" class="grow p-4">
    <div
      ...
      <div class="flex flex-col mt-2 w-full">
        <input type="text" class="w-full text-sm border rounded-sm border-gray-100 p-1" placeholder="Време за доставка"
          v-model="dogovor.vreme_dostavka" />
      </div>
      <div class="flex flex-col mt-2 w-full">
        <input type="text" class="w-full text-sm border rounded-sm border-gray-100 p-1" placeholder="Адрес на доставка"
          v-model="dogovor.adress_dostavka" />
      </div>
      <div class="flex flex-col mt-2 w-full">
        <input type="text" class="w-full text-sm border rounded-sm border-gray-100 p-1" placeholder="Коментар"
          v-model="dogovor.comment" />
      </div>
      <div class="flex flex-col mt-2 w-full">
        <input type="date" class="w-full text-sm border rounded-sm border-gray-100 p-1" placeholder="Дата на аванс 1"
          v-model="dogovor.date_avans1" />
      </div>
      <div class="flex flex-col mt-2 w-full">
        <input type="text" class="w-full text-sm border rounded-sm border-gray-100 p-1" placeholder="Сума на аванс 1"
          v-model="dogovor.price_avans1" />
      </div>
      <div class="flex flex-col mt-2 w-full">
        <select class="w-full text-sm border rounded-sm border-gray-100 p-1 bg-blue-300"
      ...
      <div class="flex items-center mt-2 w-full border border-gray-300 rounded-md shadow-md">
        <div class="bg-gray-50 w-1/2 px-2 py-1 border-l border-gray-300 rounded-l-md">
          Цена без ддс
        </div>
        <div class="bg-green-50 w-1/2 px-2 py-1 border-r border-gray-300 text-right rounded-r-md">
          {{ dogovor.price_bezdds }}
        </div>
      </div>
      <div v-for="dds in dogovor.dds" :key="dds.name"
      ...
      <div class="flex items-center font-medium mt-2 w-full border border-gray-300 rounded-md shadow-md">
        <div class="bg-gray-50 w-1/2 px-2 py-1 border-l border-gray-300 rounded-l-md">
          Крайна цена
        </div>
        <div class="bg-green-50 w-1/2 px-2 py-1 border-r border-gray-300 text-right rounded-r-md">
  ...
  <div v-if="store.state.dogovorEtap == 2" class="grow p-4">
    <div
      ...
      <div class="flex flex-col mt-5 w-full">
        <select class="w-full text-sm border rounded-sm border-gray-100 p-1 bg-blue-300" v-model="shablon1"
          @change.stop="changeDescription2()">
          <option value="">Избери от моите шаблони</option>
          <option v-for="shablon in store.state.shabloni" :key="shablon.name" :value="shablon.description">
            {{ shablon.name }}
          </option>
        </select>
      </div>
      <div class="flex flex-col mt-1 w-full">
        <textarea rows="8" class="w-full text-sm border rounded-sm border-gray-100 p-1"
          v-model="dogovor.description2"></textarea>
      </div>
      <div class="flex flex-col mt-5 w-full">
        <select class="w-full text-sm border rounded-sm border-gray-100 p-1 bg-blue-300" v-model="shablon2"
      ...
      <div class="flex flex-col mt-1 w-full">
        <textarea rows="8" class="w-full text-sm border rounded-sm border-gray-100 p-1"
          v-model="dogovor.description"></textarea>
      </div>
    </div>
  </div>
  <ModalBlank id="success-modal" :modalOpen="store.state.successUpdateDogovor"
